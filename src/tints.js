...
// Returns [r, g, b] once the thumbnail has been sampled, otherwise null (and
// starts loading it, calling `ready` when done).
export function tintOf(src, ready){
  const hit = cache.get(src);
  if (hit !== undefined) return hit;
  cache.set(src, null);
  const img = new Image();
  img.decoding = "async";
  img.onload = () => { try { cache.set(src, sample(img)); } catch (e) { cache.set(src, [236, 240, 245]); } ready(); };
