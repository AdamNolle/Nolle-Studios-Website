...
- A clipping audit checked every visible text element for sideways truncation, vertical clipping, clipping by an ancestor, and escaping the viewport. It covered the table, keys, About, shoot board, selection bar, and preview at 1920×1080, 1440×900, 1024×768, 768×1024, 844×390, 390×844, 360×740, and 320×640. Fixed: serif titles clipped their descenders (they now clip only sideways); the phone Download button was squeezed to 44 px; the keys dialog ran off 390 px phones (rows are now compact); and small landscape sheets cut the shoot title to a few letters (small sheets now stack title and date). On 667×375 and 320×640 the keys and About panels still scroll inside the glass, with a fade showing there is more.
